...
def _euclidean_distances(X, Y, squared=False):
    ...
    distances += XX
    distances += YY
    torch.maximum(distances, torch.tensor(0), out=distances)

    # Ensure that distances between vectors and themselves are set to 0.0.
    # This may not be the case due to floating point rounding errors.
    if X is Y:
        torch.fill_diagonal(distances, 0)
...
def row_norms(X, squared=False):
    ...
    norms = torch.einsum("ij,ij->i", X, X)
...
logits_selfImpl = rbf_kernel(input_rep.cuda(), label_rep.T.cuda())
print(logits)
print("==================")
